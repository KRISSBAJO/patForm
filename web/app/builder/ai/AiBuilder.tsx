...
export function AiBuilder() {
  ...
  return <main className="aiPage">
    ...
    <div className="aiPage__body">
      <div className="aiPage__intro"><span className="aiPage__eyebrow"><span className="aiPage__spark">✳</span> AI BUILDER</span><h1>Describe your process.<br/><em>Review the draft.</em></h1><p>AI drafts the form and approval flow. You check and edit them before publishing.</p></div>
      <div className="aiPage__grid">
        <section className="aiPage__card" aria-label="Describe your process">
          ...
          <label className="aiPage__label" htmlFor="ai-description">Describe the process</label>
          ...
          {jobId && <div className={`aiPage__progress ${job?.status === 'failed' ? 'aiPage__progress--failed' : ''}`}>
            <div className="aiPage__progressHeading"><span className={`aiPage__progressIcon ${job?.status === 'ready' || job?.status === 'failed' ? 'done' : ''}`} aria-hidden="true">{job?.status === 'ready' ? job.error ? '!' : '✓' : job?.status === 'failed' ? '!' : '✳'}</span><div><strong role="status" aria-live="polite">{progressTitle}</strong><p>{job?.status === 'failed' ? job.error : job?.status === 'ready' ? job.error ? `${job.error} Open the draft to fix it before publishing.` : 'Open the draft to review and edit it.' : 'You can leave this page and return with this link.'}</p></div></div>
            ...
            <div className="aiPage__progressFoot">{job?.status === 'running' || job?.status === 'queued' ? <span>Working for {Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, '0')}{elapsed >= 90 ? ' · Taking longer than expected' : ''}</span> : <span>{job?.status === 'ready' ? job.error ? 'Review required before publishing' : 'Ready to review' : 'No draft was saved'}</span>}{job?.status === 'ready' && job.draft_id && <a href={`/builder?draft=${job.draft_id}`}>Open draft →</a>}{job?.status === 'failed' && <button type="button" onClick={() => { setJobId(null); setJob(null); window.history.replaceState({}, '', '/builder/ai'); }}>Try again</button>}</div>
          </div>}
        </section>
        <aside className="aiPage__aside"><div className="aiPage__asideTop"><span className="aiPage__orb">✳</span><span>IN YOUR DRAFT</span></div><h2>Form and workflow</h2><ol><li><strong>Questions</strong><span>Fields and pages for the person submitting.</span></li><li><strong>Approvals</strong><span>Review steps, decisions and notifications.</span></li><li><strong>Checks</strong><span>Errors to fix before publishing.</span></li></ol></aside>
      </div>
    </div>
  </main>;
}
