      ...
      <div className="aiPage__grid">
        <section className="aiPage__card" aria-label="Describe your process">
          ...
          {jobId && <div className={`aiPage__progress ${job?.status === 'failed' ? 'aiPage__progress--failed' : ''}`}>
            ...
            <div className="aiPage__progressFoot">{job?.status === 'running' || job?.status === 'queued' ? <span>Working for {Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, '0')} · This can take a few minutes</span> : <span>{job?.status === 'ready' ? job.error ? 'Needs sample-check fixes before launch' : 'Ready to review' : 'No draft was published'}</span>}{job?.status === 'ready' && job.draft_id && <a href={`/builder?draft=${job.draft_id}`}>Open draft →</a>}{job?.status === 'failed' && <button type="button" onClick={() => { setJobId(null); setJob(null); window.history.replaceState({}, '', '/builder/ai'); }}>Try again</button>}</div>
          </div>}
        </section>
