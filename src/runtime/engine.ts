...
async function performEffect(
  ...
  switch (action.do) {
    ...
    case 'send_email': {
      ...
      let messageAnswers = answers;
      if ([...placeholdersIn(template.subject), ...placeholdersIn(template.body)].includes('decision_reason')) {
        const { rows } = await client.query<{ type: string; payload: { reason?: unknown } }>(
          'select type, payload from event where id = $1 and instance_id = $2',
          [args.eventId ?? null, instance.id],
        );
        const event = rows[0];
        messageAnswers = { ...answers, decision_reason: event?.type === 'approval_decided' &&
        ...
        if (unreachable.length || dropped.length) {
          await client.query(
            ...
            [
              ...
              template.key,
              [],
              render(template.subject, messageAnswers),
              [
      ...
      const subject = render(template.subject, messageAnswers);
      const body = render(template.body, messageAnswers);
