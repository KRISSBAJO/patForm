...
`npm run packs:depth` prints the measurement, thinnest first, so the remaining hundred and eight were a ranked list rather than a feeling.

The rest followed, category by category in alphabetical order, Church to Sales. Doing it the second time taught what the first fourteen had not: a condition whose trigger is the first option of its own list is required in every generated test; a question required under a condition inside a hidden section reveals whether the engine and the page check agree (they did not; see 104); and the two catalogue files needed one module for the condition helpers, because the second cannot import the first without a cycle. Two of the last hundred were typos in my own specs, an option name that did not match its list and a field key used twice, and the compiler caught both before a person could. All 122 now measure at the depth budget transfer set, every one compiles and passes its category audit, and every generated scenario passes.
